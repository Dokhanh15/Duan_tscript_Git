  ...
  return (
    <div className="login mt-5 mb-5 p-5">
      ...
      <form action="" onSubmit={handleSubmit(onSubmit)}>
        ...
        <input type="email" className="form-control w-50  "  {...register('email', { required: true, minLength: 2, maxLength: 255, })} />
        {errors.email && <p className='text-danger'>{errors.email.message}</p>}

        <label htmlFor="password">Password:</label>
        <input type="password" className="form-control w-50 " {...register('password', { required: true, minLength: 5, maxLength: 255, })} />
        {errors.password && <p className="text-danger">{errors.password.message}</p>}

        <button type="submit" className="btn btn-primary mt-3 mb-5">ĐĂNG NHẬP</button>
      </form>
      <p >Bạn chưa có tài khoản?<a href="/login">Đăng ký</a></p>
    </div>
  )
}
